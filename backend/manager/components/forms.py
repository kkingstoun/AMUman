...
class EditTaskForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "error_time": forms.DateTimeInput(
                attrs={"class": "form-control", "type": "datetime-local"}
            ),
...
class AddTaskForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            ),  # Upewnij się, że to TextInput
            "priority": forms.Select(
                choices=[("slow", "Slow"), ("normal", "Normal"), ("fast", "Fast")],
                attrs={"class": "form-control"},
            ),
            "gpu_partition": forms.Select(
                choices=[("normal", "Normal"), ("high", "High")],
                attrs={"class": "form-control"},
            ),
            "est": forms.TextInput(
                attrs={"class": "form-control"}
            ),  # Jeśli est jest czasem, możesz potrzebować specjalnego widgetu
        }
        help_texts = {"est": "Estimated simulation time (in hours)"}
        ...
        self.helper.layout = Layout(
            Div(
                Div(
                    ...
                    HTML(
                        '<label class="form-check-label" for="id_queue_watchdog">Default switch checkbox input</label>'
                    ),
