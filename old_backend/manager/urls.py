...
urlpatterns = [
    ...
    path(
        "task/<action>/<int:task_id>/", TaskManagerView.as_view(), name="task_action_id"
    ),
    ...
    path(
        "nodes/refresh_all_nodes/", NodeListView.as_view(), name="refresh_all_nodes"
    ),  # Lista wszystkich węzłów
    path("nodes/<int:node_id>/", NodeListView.as_view(), name="manage_node"),
    path(
        "nodes/<int:node_id>/refresh_gpus/", NodeListView.as_view(), name="refresh_gpus"
    ),
    path(
        "nodes/<int:node_id>/refresh_gpus_ajax/",
        NodeListView.refresh_gpus_ajax,
        name="refresh_gpus_ajax",
    ),
